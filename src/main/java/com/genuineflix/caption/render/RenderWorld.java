...
public class RenderWorld {
	...
	private static void drawCaption(final CaptionWorld caption, final float partialTicks) {
		...
		final double translateX = rm.viewerPosX - (caption.prevPosX + (caption.posX - caption.prevPosX) * partialTicks);
		final double translateY = rm.viewerPosY - (caption.prevPosY + (caption.posY - caption.prevPosY) * partialTicks)
				- caption.size;
		final double translateZ = rm.viewerPosZ - (caption.prevPosZ + (caption.posZ - caption.prevPosZ) * partialTicks);
		GL11.glTranslated(-translateX, -translateY, -translateZ);
		GL11.glRotated(rm.playerViewY + 180, 0, -0.5, 0);
		...
		if (alpha < 5)
			alpha = 5;
		RenderHelper.drawTooltip(x, y, w, h, (alpha << 24) | (RenderHelper.mainColor & 0xFFFFFF),
				(alpha << 24) | (RenderHelper.outlineColor & 0xFFFFFF),
				(alpha << 24) | (RenderHelper.secondaryColor & 0xFFFFFF));
		...
		GL11.glRotated(rm.playerViewX, 1, 0, 0);
		GL11.glRotated(rm.playerViewY - 180, 0, 1, 0);
		GL11.glTranslated(translateX, translateY, translateZ);
	}
